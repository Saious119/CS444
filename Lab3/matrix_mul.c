...
void *matrix_mul_thread(struct mtx_mul_params *params) {
	 ...
	 * params->B, params->M, params->row, etc. */
        ...
        /*TODO for students*/
	int col;
	for(params->row = 0; params->row < params->M; params->row++){
		params->C[params->row] = 0.0;
		for(col =0; col < params->N; col++){
			params->C[params->row] += params->A[params->row][col] * params->B[col];
		}
	}	
}
